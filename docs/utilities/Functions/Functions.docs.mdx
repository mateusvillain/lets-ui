...
export const colorFns = [
  ...
  {
    fn: 'state($state, $bg?)',
    desc: 'Background do componente no estado: a tinta misturada ao fundo de repouso',
    params: [
  ...
  {
    fn: 'state-tint()',
    desc: 'A tinta de interação — preta nos temas claros, branca nos escuros',
    params: [],
  },
  {
    fn: 'state-amount($state)',
    desc: 'Quanto da tinta misturar, como número de 0 a 1',
    params: [
      { label: '$state', values: ['hover', 'pressed'] },
    ],
  },
...
.btn--primary {
  ...
  // repouso entra pelo mixin e as regras de estado misturam a tinta nele.
  // `states()` já embute tudo isso.
